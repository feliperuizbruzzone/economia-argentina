"""Validate the second P0 long extract for Ganancias Sociedades."""
...
from afip_ganancias_sociedades import P0_ACTIVITY_COSTS_TABLE_ID  # noqa: E402
...
REPORT_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_2_1_validation.md"
)
DETAIL_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_2_1_validation_counts.csv"
)
...
OVERLAP_WITH_SUMMARY_TABLE = {
    "presentaciones_total",
    "presentaciones_con_ventas",
    "ventas_bienes_servicios_locaciones_netas",
}
KNOWN_MISSING_BY_FISCAL_YEAR = {
    "2017": {"ventas_bienes_servicios_locaciones_netas"},
}
...
def _validate_overlap_with_summary(
    ...
    missing_from_current = set(summary) - set(current)
    missing_from_summary = sorted(set(current) - set(summary))
    for key in sorted(missing_from_current):
        fiscal_year, _, variable_name = key
        if variable_name in KNOWN_MISSING_BY_FISCAL_YEAR.get(fiscal_year, set()):
            warnings.append(
                f"Overlap check: fiscal year {fiscal_year} source omits `{variable_name}` "
                "in table 2.3.1.1.2.1; reference table keeps the value."
            )
            missing_from_current.remove(key)
    ...
    if missing_from_summary:
        failures.append(
            ...
            f"{len(missing_from_summary)}"
        )

    overlap_keys = set(current) & set(summary)
    mismatches = [
        key
        for key in overlap_keys
        if current[key] != summary[key]
    ]
    if mismatches:
...
def _validate(
    ...
    if not rows:
        return ["No rows found in the P0 2.3.1.1.2.1 long extract."], warnings, report
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if row["source_table_id"] != P0_ACTIVITY_COSTS_TABLE_ID:
    ...
    overlap_failures, overlap_warnings, overlap_rows_checked = _validate_overlap_with_summary(
        rows,
        summary_rows,
    )
    ...
    report.extend(
        [
            "# Ganancias Sociedades P0 2.3.1.1.2.1 Long Extract Validation",
            "",
            "Scope: table `2.3.1.1.2.1`, `P0_new_xls_detail`, activity dimension.",
            "",
            f"- rows_validated: {len(rows)}",
            f"- fiscal_years: {', '.join(fiscal_years)}",
            f"- variables: {', '.join(sorted(variables))}",
            f"- overlap_rows_checked_against_2_3_1_1_1: {overlap_rows_checked}",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P0_231121_LONG_PATH)
