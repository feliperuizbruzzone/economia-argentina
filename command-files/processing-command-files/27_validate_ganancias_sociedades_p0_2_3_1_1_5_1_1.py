"""Validate the twelfth P0 long extract for Ganancias Sociedades."""
...
from afip_ganancias_sociedades import P0_ACTIVITY_CASH_ASSETS_TABLE_ID  # noqa: E402
...
REPORT_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_5_1_1_validation.md"
)
DETAIL_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_5_1_1_validation_counts.csv"
)
...
def _validate_overlap_with_assets_table(
    ...
    known_missing_from_current = [
        key for key in missing_from_current if key[0] in KNOWN_SOURCE_MISSING_FISCAL_YEARS
    ]
    ...
    if known_missing_from_current:
        ...
        warnings.append(
            "Overlap check: table 2.3.1.1.5.1.1 is source-missing for early "
            f"fiscal years; missing overlap rows against 2.3.1.1.5.1: "
            f"{len(known_missing_from_current)} ({detail})."
        )
    if unexpected_missing_from_current:
        failures.append(
            "Overlap check: rows missing from 2.3.1.1.5.1.1 against 2.3.1.1.5.1: "
...
def _validate_component_identity(rows: list[dict[str, str]]) -> list[str]:
    failures: list[str] = []
    by_dimension: dict[tuple[str, str], dict[str, Decimal]] = defaultdict(dict)
    required_variables = COMPONENT_VARIABLES | {"activo_disponibilidades"}
    for row in rows:
    ...
    for key, values in sorted(by_dimension.items()):
        ...
        if missing:
            failures.append(
                ...
                f"cash-assets identity missing variables: {', '.join(missing)}"
            )
            continue

        component_sum = sum(values[variable_name] for variable_name in COMPONENT_VARIABLES)
        if abs(component_sum - values["activo_disponibilidades"]) > MONETARY_TOLERANCE:
            failures.append(
                f"fiscal_year={fiscal_year} dimension_value={dimension_value}: "
                "cash-asset components do not sum to total cash assets."
            )
    return failures
...
def _validate(
    ...
    if not rows:
        return ["No rows found in the P0 2.3.1.1.5.1.1 long extract."], warnings, report
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if row["source_table_id"] != P0_ACTIVITY_CASH_ASSETS_TABLE_ID:
    ...
    overlap_failures, overlap_warnings, overlap_rows = _validate_overlap_with_assets_table(
        rows,
        assets_rows,
    )
    identity_failures = _validate_component_identity(rows)
    ...
    report.extend(
        [
            "# Ganancias Sociedades P0 2.3.1.1.5.1.1 Long Extract Validation",
            "",
            "Scope: table `2.3.1.1.5.1.1`, `P0_new_xls_detail`, activity dimension.",
            "",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P0_2311511_LONG_PATH)
