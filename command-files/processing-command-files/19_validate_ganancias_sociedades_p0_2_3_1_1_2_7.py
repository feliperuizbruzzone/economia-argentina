"""Validate the eighth P0 long extract for Ganancias Sociedades."""
...
from afip_ganancias_sociedades import P0_ACTIVITY_ACCOUNTING_RESULT_TABLE_ID  # noqa: E402
...
REPORT_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_2_7_validation.md"
)
DETAIL_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_2_7_validation_counts.csv"
)
...
FISCAL_2022_ONLY_CASE_VARIABLES = {
    "resultado_final_contable_juegos_azar_apuestas_casos",
}
FISCAL_2022_ONLY_MONETARY_VARIABLES = {
    "resultado_final_contable_juegos_azar_apuestas",
}
CASE_VARIABLES = COMMON_CASE_VARIABLES | FISCAL_2022_ONLY_CASE_VARIABLES
MONETARY_VARIABLES = COMMON_MONETARY_VARIABLES | FISCAL_2022_ONLY_MONETARY_VARIABLES
COMMON_VARIABLES = COMMON_CASE_VARIABLES | COMMON_MONETARY_VARIABLES
FISCAL_2022_ONLY_VARIABLES = (
    FISCAL_2022_ONLY_CASE_VARIABLES | FISCAL_2022_ONLY_MONETARY_VARIABLES
)
EXPECTED_VARIABLES = COMMON_VARIABLES | FISCAL_2022_ONLY_VARIABLES
OVERLAP_WITH_SUMMARY_TABLE = {"presentaciones_total"}
KNOWN_ACTIVITY_CODE_ALIASES_WITH_SUMMARY = {
    (
        "2019",
        "352;353",
        "presentaciones_total",
...
def _expected_variables_for_year(fiscal_year: str) -> set[str]:
    if fiscal_year == "2022":
...
def _validate_overlap_with_summary_table(
    ...
    missing_from_current = set(reference) - set(current)
    missing_from_reference = set(current) - set(reference)
    alias_matches = 0
    for current_key, reference_key in KNOWN_ACTIVITY_CODE_ALIASES_WITH_SUMMARY.items():
        if current_key in missing_from_reference and reference_key in missing_from_current:
            if current[current_key] == reference[reference_key]:
                missing_from_reference.remove(current_key)
                missing_from_current.remove(reference_key)
                alias_matches += 1
                warnings.append(
                    "Overlap check: fiscal year 2019 uses source activity code "
                    "`352;353` in table 2.3.1.1.2.7 but `352` in table 2.3.1.1.1; "
                    "values match and codes are preserved without homologation."
                )

    if missing_from_current:
        failures.append(
            "Overlap check: rows missing from 2.3.1.1.2.7 against 2.3.1.1.1: "
    ...
    return failures, warnings, len(overlap_keys) + alias_matches
...
def _validate(
    ...
    if not rows:
        return ["No rows found in the P0 2.3.1.1.2.7 long extract."], warnings, report

    variables = {row["variable_name"] for row in rows}
    missing_common_variables = sorted(COMMON_VARIABLES - variables)
    extra_variables = sorted(variables - EXPECTED_VARIABLES)
    if missing_common_variables:
        failures.append(f"Missing common variables: {', '.join(missing_common_variables)}")
    if extra_variables:
        failures.append(f"Unexpected variables: {', '.join(extra_variables)}")
    if FISCAL_2022_ONLY_VARIABLES <= variables:
        warnings.append(
            "Fiscal year 2022 includes two source-specific games/gambling "
            "accounting-result variables not present in fiscal years 2020-2021."
        )
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if row["source_table_id"] != P0_ACTIVITY_ACCOUNTING_RESULT_TABLE_ID:
        ...
        if row["variable_name"] in FISCAL_2022_ONLY_VARIABLES and row["fiscal_year"] != "2022":
            failures.append(f"{label}: 2022-only variable appears outside fiscal year 2022")
    ...
    for fiscal_year in fiscal_years:
        expected_year_variables = _expected_variables_for_year(fiscal_year)
        year_rows = [row for row in rows if row["fiscal_year"] == fiscal_year]
        year_variables = {row["variable_name"] for row in year_rows}
        missing = sorted(expected_year_variables - year_variables)
        unexpected = sorted(year_variables - expected_year_variables)
        if missing:
            failures.append(f"Fiscal year {fiscal_year} missing variables: {', '.join(missing)}")
        if unexpected:
            failures.append(f"Fiscal year {fiscal_year} unexpected variables: {', '.join(unexpected)}")
        ...
        for variable_name in expected_year_variables:
            total_rows = [
                row
                for row in year_rows
    ...
    overlap_failures, overlap_warnings, overlap_rows = _validate_overlap_with_summary_table(
        rows,
        summary_rows,
    )
    ...
    report.extend(
        [
            "# Ganancias Sociedades P0 2.3.1.1.2.7 Long Extract Validation",
            "",
            "Scope: table `2.3.1.1.2.7`, `P0_new_xls_detail`, activity dimension.",
            "",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P0_231127_LONG_PATH)
