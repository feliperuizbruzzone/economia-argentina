"""Validate the P2 Ganancias Sociedades HTML long extract."""
...
from afip_ganancias_sociedades import P2_ACTIVITY_TABLE_IDS, normalize_text  # noqa: E402
...
REPORT_PATH = VALIDATION_REPORTS_DIR / "ganancias_sociedades_p2_long_validation.md"
DETAIL_PATH = VALIDATION_REPORTS_DIR / "ganancias_sociedades_p2_long_counts.csv"
...
def _validate_rows(
    ...
    if not rows:
        return ["No rows found in the P2 long extract."], warnings, report
    ...
    if observed_tables != expected_tables:
        failures.append(
            "Extract source tables differ from expected P2 tables: "
            f"missing={sorted(expected_tables - observed_tables)}, "
            f"extra={sorted(observed_tables - expected_tables)}"
        )
    ...
    report.extend(
        [
            "# Ganancias Sociedades P2 Long Validation",
            "",
            "Scope: `P2_old_html_detail_millions`, publication year 2013, fiscal year 2012.",
            "",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P2_LONG_PATH)
    inventory_rows = _load_csv(GANANCIAS_SOCIEDADES_P2_INVENTORY_PATH)
