"""Validate the P3 Ganancias Sociedades HTML long extract."""
...
from afip_ganancias_sociedades import P3_ACTIVITY_TABLE_IDS, normalize_text  # noqa: E402
...
REPORT_PATH = VALIDATION_REPORTS_DIR / "ganancias_sociedades_p3_long_validation.md"
DETAIL_PATH = VALIDATION_REPORTS_DIR / "ganancias_sociedades_p3_long_counts.csv"
MONETARY_TOLERANCE = Decimal("0.000001")
EXPECTED_FISCAL_BY_PUBLICATION = {
    "2009": "2008",
    "2010": "2009",
    "2011": "2010",
    "2012": "2011",
}
FISCAL_2008_MISSING_TABLES = {
    "2.3.1.1.2.5",
    "2.3.1.1.2.6",
    "2.3.1.1.2.7",
}
...
def _expected_inventory_pairs() -> set[tuple[str, str]]:
    ...
    for publication_year in EXPECTED_FISCAL_BY_PUBLICATION:
        for table_id in P3_ACTIVITY_TABLE_IDS:
            if publication_year == "2009" and table_id in FISCAL_2008_MISSING_TABLES:
                continue
            expected.add((publication_year, table_id))
    return expected
...
def _validate_inventory(inventory_rows: list[dict[str, str]]) -> tuple[list[str], list[str]]:
    ...
    observed_pairs = {
        (row["publication_year"], row["source_table_id"]) for row in inventory_rows
    }
    if observed_pairs != expected_pairs:
        missing = sorted(expected_pairs - observed_pairs)
        extra = sorted(observed_pairs - expected_pairs)
        if missing:
            warnings.append(f"Inventory missing expected P3 table-year pairs: {missing}")
        if extra:
            failures.append(f"Inventory has unexpected P3 table-year pairs: {extra}")

    for row in inventory_rows:
        label = f"inventory publication={row['publication_year']} table={row['source_table_id']}"
        expected_fiscal_year = EXPECTED_FISCAL_BY_PUBLICATION.get(row["publication_year"])
        if row["period_id"] != "P3_old_html_detail_thousands":
            failures.append(f"{label}: period_id={row['period_id']}")
        if expected_fiscal_year is None:
            failures.append(f"{label}: unexpected publication_year={row['publication_year']}")
        elif row["fiscal_year"] != expected_fiscal_year:
        ...
        if not row["source_table_path"].endswith("_archivos/sheet002.htm"):
            warnings.append(
                f"{label}: source path is not a `sheet002.htm` detail sheet: "
                f"{row['source_table_path']}"
            )
        if "fiscal_2008_source_table_id_semantic_shift" in row["notes"]:
            warnings.append(f"{label}: fiscal 2008 table id has shifted semantics.")
        if "fiscal_2008_costs_otros_component" in row["notes"]:
            warnings.append(f"{label}: fiscal 2008 reports `OTROS` cost component.")
...
def _validate_rows(
    ...
    if not rows:
        return ["No rows found in the P3 long extract."], warnings, report
    ...
    extract_pairs = {(row["publication_year"], row["source_table_id"]) for row in rows}
    if extract_pairs != inventory_pairs:
        failures.append(
            "Extract table-year pairs differ from inventory: "
    ...
    expected_tables = set(P3_ACTIVITY_TABLE_IDS)
    for row_number, row in enumerate(rows, start=2):
        label = (
            f"csv row {row_number}: publication={row['publication_year']} "
        ...
        if row["period_id"] != "P3_old_html_detail_thousands":
            failures.append(f"{label}: period_id={row['period_id']}")
        if expected_fiscal_year is None or row["fiscal_year"] != expected_fiscal_year:
            failures.append(f"{label}: fiscal_year={row['fiscal_year']}")
    ...
    report.extend(
        [
            "# Ganancias Sociedades P3 Long Validation",
            "",
            "Scope: `P3_old_html_detail_thousands`, publication years 2009-2012, fiscal years 2008-2011.",
            "",
            f"- rows_validated: {len(rows)}",
            f"- source_table_year_pairs: {len(extract_pairs)}",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P3_LONG_PATH)
    inventory_rows = _load_csv(GANANCIAS_SOCIEDADES_P3_INVENTORY_PATH)
