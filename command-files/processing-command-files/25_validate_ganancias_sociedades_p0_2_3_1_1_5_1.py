"""Validate the eleventh P0 long extract for Ganancias Sociedades."""
...
from afip_ganancias_sociedades import P0_ACTIVITY_ASSETS_TABLE_ID  # noqa: E402
...
REPORT_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_5_1_validation.md"
)
DETAIL_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_5_1_validation_counts.csv"
)
...
def _validate_overlap_with_summary_table(
    ...
    if missing_from_current:
        failures.append(
            "Overlap check: rows missing from 2.3.1.1.5.1 against 2.3.1.1.1: "
...
def _validate_component_identity(rows: list[dict[str, str]]) -> list[str]:
    ...
    required_variables = COMPONENT_VARIABLES | {"activo_total"}
    for row in rows:
    ...
    for key, values in sorted(by_dimension.items()):
        ...
        if missing:
            failures.append(
                ...
                f"asset identity missing variables: {', '.join(missing)}"
            )
            continue

        component_sum = sum(values[variable_name] for variable_name in COMPONENT_VARIABLES)
        if abs(component_sum - values["activo_total"]) > MONETARY_TOLERANCE:
            failures.append(
                f"fiscal_year={fiscal_year} dimension_value={dimension_value}: "
                "asset components do not sum to total assets."
            )
    return failures
...
def _validate(
    ...
    if not rows:
        return ["No rows found in the P0 2.3.1.1.5.1 long extract."], warnings, report
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if row["source_table_id"] != P0_ACTIVITY_ASSETS_TABLE_ID:
    ...
    report.extend(
        [
            "# Ganancias Sociedades P0 2.3.1.1.5.1 Long Extract Validation",
            "",
            "Scope: table `2.3.1.1.5.1`, `P0_new_xls_detail`, activity dimension.",
            "",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P0_231151_LONG_PATH)
