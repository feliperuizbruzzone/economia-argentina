"""Validate the ninth P0 long extract for Ganancias Sociedades."""
...
from afip_ganancias_sociedades import P0_ACTIVITY_TAX_RESULT_TABLE_ID  # noqa: E402
...
REPORT_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_3_validation.md"
)
DETAIL_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_3_validation_counts.csv"
)
...
KNOWN_MISSING_BY_FISCAL_YEAR = {
    "2014": {"quebranto_computable", "quebranto_computable_casos"},
    "2015": {"quebranto_computable", "quebranto_computable_casos"},
    "2016": {"quebranto_computable", "quebranto_computable_casos"},
    "2017": {"quebranto_computable", "quebranto_computable_casos"},
}
...
def _validate_overlap_with_summary_table(
    ...
    if missing_from_current:
        failures.append(
            "Overlap check: rows missing from 2.3.1.1.3 against 2.3.1.1.1: "
...
def _validate(
    ...
    if not rows:
        return ["No rows found in the P0 2.3.1.1.3 long extract."], warnings, report
    ...
    if "2020" in {row["fiscal_year"] for row in rows}:
        warnings.append(
            "Fiscal year 2020 source header has incomplete `Casos`/`Importe` "
            "labels for `resultado_impositivo_perdida`; extraction maps those "
            "two columns by the result-impositivo block and adjacent-column evidence."
        )
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if row["source_table_id"] != P0_ACTIVITY_TAX_RESULT_TABLE_ID:
    ...
    report.extend(
        [
            "# Ganancias Sociedades P0 2.3.1.1.3 Long Extract Validation",
            "",
            "Scope: table `2.3.1.1.3`, `P0_new_xls_detail`, activity dimension.",
            "",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P0_23113_LONG_PATH)
