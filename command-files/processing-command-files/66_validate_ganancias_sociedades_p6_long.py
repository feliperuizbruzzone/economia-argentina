"""Validate the P6 Ganancias Sociedades legacy direct-XLS long extract."""
...
REPORT_PATH = VALIDATION_REPORTS_DIR / "ganancias_sociedades_p6_long_validation.md"
DETAIL_PATH = VALIDATION_REPORTS_DIR / "ganancias_sociedades_p6_long_counts.csv"
MONETARY_TOLERANCE = Decimal("0.000001")
EXPECTED_FISCAL_PUBLICATION = {"1997": "1998", "1998": "1999"}
EXPECTED_FISCAL_YEARS = ("1997", "1998")
...
def _expected_inventory_pairs() -> set[tuple[str, str]]:
    ...
    for table_id in P6_ACTIVITY_TABLE_IDS_1997:
        expected.add(("1997", table_id))
    for table_id in P6_ACTIVITY_TABLE_IDS_1998:
        expected.add(("1998", table_id))
    return expected
...
def _validate_inventory(inventory_rows: list[dict[str, str]]) -> tuple[list[str], list[str]]:
    ...
    if observed_pairs != expected_pairs:
        ...
        if missing:
            failures.append(f"Inventory missing expected P6 fiscal/table pairs: {missing}")
        if extra:
            failures.append(f"Inventory has unexpected P6 fiscal/table pairs: {extra}")

    for row in inventory_rows:
        label = f"inventory fiscal={row['fiscal_year']} table={row['source_table_id']}"
        if row["period_id"] != "P6_legacy_broad_activity":
        ...
        expected_publication = EXPECTED_FISCAL_PUBLICATION.get(row["fiscal_year"])
        if row["publication_year"] != expected_publication:
            failures.append(f"{label}: publication_year={row['publication_year']}")
        if row["format"] != "direct_xls":
        ...
        if row["fiscal_year"] == "1997" and row["has_activity_3digit"] != "False":
            failures.append(f"{label}: fiscal 1997 should not have 3-digit detail.")
        if row["fiscal_year"] == "1998" and row["has_activity_3digit"] != "True":
            failures.append(f"{label}: fiscal 1998 detail continuation was not detected.")
        if row["fiscal_year"] == "1998" and not row["source_table_path"].endswith("_2.xls"):
            warnings.append(f"{label}: source path is not an `_2.xls` detail file.")
        if "legacy_four_table_schema" in row["notes"]:
            warnings.append(f"{label}: legacy four-table schema.")
        if "broad_activity_only" in row["notes"]:
            warnings.append(f"{label}: fiscal 1997 is broad-activity only.")
        if "p6_1998_has_3digit_detail" in row["notes"]:
            warnings.append(f"{label}: fiscal 1998 has 3-digit detail in `_2.xls`.")
        ...
        if "tax_adjustment_columns_only_in_fiscal_1997" in row["notes"]:
            warnings.append(f"{label}: tax-adjustment columns only exist for fiscal 1997.")
...
def _validate_rows(
    ...
    if not rows:
        return ["No rows found in the P6 long extract."], warnings, report
    ...
    levels_by_year: dict[str, Counter[str]] = defaultdict(Counter)
    for row in rows:
        levels_by_year[row["fiscal_year"]][row["activity_level"]] += 1

    if levels_by_year["1997"].get("activity_3digit", 0):
        failures.append("Fiscal 1997 unexpectedly contains activity_3digit rows.")
    if not levels_by_year["1998"].get("activity_3digit", 0):
        failures.append("Fiscal 1998 has no activity_3digit rows.")
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if row["publication_year"] != EXPECTED_FISCAL_PUBLICATION.get(row["fiscal_year"]):
            failures.append(f"{label}: publication_year={row['publication_year']}")
        if row["dimension_type"] != "actividad_economica":
            failures.append(f"{label}: dimension_type={row['dimension_type']}")
        ...
        if row["activity_level"] == "broad_activity" and not row["activity_code"]:
            failures.append(f"{label}: blank broad activity code")
    ...
    report.extend(
        [
            "# Ganancias Sociedades P6 Long Validation",
            "",
            "Scope: `P6_legacy_broad_activity`, publication years 1998-1999, fiscal years 1997-1998.",
            "",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P6_LONG_PATH)
    inventory_rows = _load_csv(GANANCIAS_SOCIEDADES_P6_INVENTORY_PATH)
