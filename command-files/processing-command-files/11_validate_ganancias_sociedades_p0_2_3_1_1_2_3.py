"""Validate the fourth P0 long extract for Ganancias Sociedades."""
...
from afip_ganancias_sociedades import P0_ACTIVITY_COST_LINKED_EXPENSES_TABLE_ID  # noqa: E402
...
REPORT_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_2_3_validation.md"
)
DETAIL_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_2_3_validation_counts.csv"
)
...
def _validate_overlap_with_costs_table(
    ...
    if missing_from_current:
        failures.append(
            "Overlap check: rows missing from 2.3.1.1.2.3 against 2.3.1.1.2.1: "
...
def _validate_component_identity(rows: list[dict[str, str]]) -> list[str]:
    ...
    for key, values in sorted(by_dimension.items()):
        ...
        if abs(component_sum - values["gastos_vinculados_al_costo"]) > MONETARY_TOLERANCE:
            failures.append(
                f"fiscal_year={fiscal_year} dimension_value={dimension_value}: "
                "cost-linked expense components do not sum to total."
            )
    return failures
...
def _validate(
    ...
    if not rows:
        return ["No rows found in the P0 2.3.1.1.2.3 long extract."], warnings, report
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if row["source_table_id"] != P0_ACTIVITY_COST_LINKED_EXPENSES_TABLE_ID:
    ...
    report.extend(
        [
            "# Ganancias Sociedades P0 2.3.1.1.2.3 Long Extract Validation",
            "",
            "Scope: table `2.3.1.1.2.3`, `P0_new_xls_detail`, activity dimension.",
            "",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P0_231123_LONG_PATH)
