"""Validate the third P0 long extract for Ganancias Sociedades."""
...
from afip_ganancias_sociedades import P0_ACTIVITY_SALES_DETAIL_TABLE_ID  # noqa: E402
...
REPORT_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_2_2_validation.md"
)
DETAIL_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_2_2_validation_counts.csv"
)
...
KNOWN_MISSING_BY_FISCAL_YEAR = {
    "2014": EARLY_SALES_DETAIL_MISSING,
    "2015": EARLY_SALES_DETAIL_MISSING,
    "2016": {
        "exportaciones_baja_nula_imposicion_sin_detraer_derechos",
        "exportaciones_baja_nula_imposicion_sin_detraer_derechos_casos",
    },
}
KNOWN_NET_PLUS_DUTIES_SOURCE_ANOMALIES = {
    ("2017", "TOTAL"),
    ("2017", "N"),
    ("2017", "773"),
}
...
def _validate_overlap(
    ...
    current = {
        ...
        for row in rows
        if row["variable_name"] in OVERLAP_WITH_PREVIOUS_TABLES
    }
    reference = {
        (row["fiscal_year"], row["dimension_value"], row["variable_name"]): row["value"]
    ...
    if missing_from_current:
        failures.append(
            f"Overlap check: rows missing from 2.3.1.1.2.2 against {reference_table_id}: "
            f"{len(missing_from_current)}"
        )
    if missing_from_reference:
        failures.append(
            f"Overlap check: rows missing from {reference_table_id}: "
    ...
    if mismatches:
        failures.append(
            f"Overlap check: value mismatches with {reference_table_id}: {len(mismatches)}"
        )

    return failures, len(overlap_keys)


def _validate_sales_identities(rows: list[dict[str, str]]) -> tuple[list[str], list[str]]:
    failures: list[str] = []
    warnings: list[str] = []
    ...
    for key, values in sorted(by_dimension.items()):
        ...
        label = f"fiscal_year={fiscal_year} dimension_value={dimension_value}"
        required = {
            "presentaciones_con_ventas",
            "total_ventas_servicios_locaciones_ejercicio_casos",
            "ventas_bienes_servicios_locaciones_netas",
        ...
        if missing:
            if missing <= KNOWN_MISSING_BY_FISCAL_YEAR.get(fiscal_year, set()):
                continue
            failures.append(f"{label}: sales identity missing variables: {', '.join(sorted(missing))}.")
            continue
        if (
            values["presentaciones_con_ventas"]
            != values["total_ventas_servicios_locaciones_ejercicio_casos"]
        ):
            failures.append(
                f"{label}: presentaciones_con_ventas differs from total sales cases."
            )

        net_plus_duties = (
            values["ventas_bienes_servicios_locaciones_netas"]
            + values["derechos_exportacion"]
        )
        if (
            abs(net_plus_duties - values["total_ventas_servicios_locaciones_ejercicio"])
            > MONETARY_TOLERANCE
        ):
            if (fiscal_year, dimension_value) in KNOWN_NET_PLUS_DUTIES_SOURCE_ANOMALIES:
                warnings.append(
                    f"{label}: source anomaly; net sales plus export duties differs "
                    "from total sales, while detailed components reconcile."
                )
                continue
            failures.append(
                f"{label}: net sales plus export duties differs from total sales."
            )

        components_total = sum(values[variable_name] for variable_name in SALES_COMPONENTS)
        if (
            abs(components_total - values["total_ventas_servicios_locaciones_ejercicio"])
            > MONETARY_TOLERANCE
        ):
            failures.append(
                f"{label}: detailed sales components do not sum to total sales."
            )
...
def _validate(
    ...
    if not rows:
        return ["No rows found in the P0 2.3.1.1.2.2 long extract."], warnings, report
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if row["source_table_id"] != P0_ACTIVITY_SALES_DETAIL_TABLE_ID:
    ...
    summary_failures, summary_overlap_rows = _validate_overlap(rows, summary_rows, "2.3.1.1.1")
    costs_failures, costs_overlap_rows = _validate_overlap(rows, costs_rows, "2.3.1.1.2.1")
    failures.extend(summary_failures)
    failures.extend(costs_failures)
    identity_failures, identity_warnings = _validate_sales_identities(rows)
    ...
    report.extend(
        [
            "# Ganancias Sociedades P0 2.3.1.1.2.2 Long Extract Validation",
            "",
            "Scope: table `2.3.1.1.2.2`, `P0_new_xls_detail`, activity dimension.",
            "",
            f"- rows_validated: {len(rows)}",
            f"- fiscal_years: {', '.join(fiscal_years)}",
            f"- variables: {', '.join(sorted(variables))}",
            f"- overlap_rows_checked_against_2_3_1_1_1: {summary_overlap_rows}",
            f"- overlap_rows_checked_against_2_3_1_1_2_1: {costs_overlap_rows}",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P0_231122_LONG_PATH)
    ...
    _write_detail(rows)
    failures, warnings, report = _validate(rows, summary_rows, costs_rows)
