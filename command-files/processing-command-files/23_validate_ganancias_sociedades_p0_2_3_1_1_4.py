"""Validate the tenth P0 long extract for Ganancias Sociedades."""
...
from afip_ganancias_sociedades import P0_ACTIVITY_TAX_DETERMINATION_TABLE_ID  # noqa: E402
...
REPORT_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_4_validation.md"
)
DETAIL_PATH = (
    VALIDATION_REPORTS_DIR
    / "ganancias_sociedades_p0_2_3_1_1_4_validation_counts.csv"
)
...
SUMMARY_OVERLAP_VARIABLES = {
    "presentaciones_total": "presentaciones_total",
    "resultado_neto_final_utilidad": "ganancia_neta_imponible",
...
def _values_match(current_variable: str, current: str, reference: str) -> bool:
    if current_variable in MONETARY_VARIABLES:
...
def _validate_overlap_with_summary_table(
    ...
    current = {
        ...
        for row in rows
        if row["variable_name"] in SUMMARY_OVERLAP_VARIABLES
    }
    reference = {
        (row["fiscal_year"], row["dimension_value"], variable_name): row["value"]
        for row in summary_rows
        for variable_name, summary_variable in SUMMARY_OVERLAP_VARIABLES.items()
        if row["variable_name"] == summary_variable
    }
    ...
    if missing_from_current:
        failures.append(
            "Overlap check: rows missing from 2.3.1.1.4 against 2.3.1.1.1: "
...
def _validate(
    ...
    if not rows:
        return ["No rows found in the P0 2.3.1.1.4 long extract."], warnings, report
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if row["source_table_id"] != P0_ACTIVITY_TAX_DETERMINATION_TABLE_ID:
    ...
    report.extend(
        [
            "# Ganancias Sociedades P0 2.3.1.1.4 Long Extract Validation",
            "",
            "Scope: table `2.3.1.1.4`, `P0_new_xls_detail`, activity dimension.",
            "",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P0_23114_LONG_PATH)
