"""Validate the P4 Ganancias Sociedades CAB/XLS long extract."""
...
from afip_ganancias_sociedades import P4_ACTIVITY_TABLE_IDS, normalize_text  # noqa: E402
...
REPORT_PATH = VALIDATION_REPORTS_DIR / "ganancias_sociedades_p4_long_validation.md"
DETAIL_PATH = VALIDATION_REPORTS_DIR / "ganancias_sociedades_p4_long_counts.csv"
...
def _validate_inventory(inventory_rows: list[dict[str, str]]) -> tuple[list[str], list[str]]:
    ...
    if observed_pairs != expected_pairs:
        ...
        if missing:
            failures.append(f"Inventory missing expected P4 table-year pairs: {missing}")
        if extra:
            failures.append(f"Inventory has unexpected P4 table-year pairs: {extra}")
    ...
    for row in inventory_rows:
        ...
        if row["period_id"] != "P4_old_cab_xls_detail_thousands":
        ...
        if "old_result_table_numbering" in row["notes"]:
            warnings.append(f"{label}: old result table numbering maps to later semantics.")
...
def _validate_rows(
    ...
    if not rows:
        return ["No rows found in the P4 long extract."], warnings, report
    ...
    for dimension_key, variable_names in sorted(variables_by_dimension.items()):
        ...
        table_year = (fiscal_year, source_table_id)
        expected_count = len(variables_by_table_year[table_year])
    ...
    expected_tables = set(P4_ACTIVITY_TABLE_IDS)
    ...
    report.extend(
        [
            "# Ganancias Sociedades P4 Long Validation",
            "",
            "Scope: `P4_old_cab_xls_detail_thousands`, publication years 2003-2008, fiscal years 2002-2007.",
            "",
...
def main() -> None:
    rows = _load_csv(GANANCIAS_SOCIEDADES_P4_LONG_PATH)
    inventory_rows = _load_csv(GANANCIAS_SOCIEDADES_P4_INVENTORY_PATH)
